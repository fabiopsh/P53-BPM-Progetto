...
\usepackage{float}
\usepackage{caption}
\captionsetup{font=small,labelfont=bf,margin=1cm,justification=centering}
\usepackage{booktabs}
...
% ------- figura larga, forzata a occupare tutta la larghezza del testo -------
% #1 = percorso immagine, #2 = didascalia, #3 = label
...
\begin{titlepage}
	...
	{\large Laurea Magistrale in Informatica\par}
...
con Camunda Modeler; le reti di workflow sono state ottenute per Alex e Bob tramite bpmn2petrinet.com e per la collaboration manualmente tramite WoPeD. La relazione descrive i modelli prodotti, la
...
risposta di Bob che puó essere: accettazione, controproposta o annullamento. Raggiunto l'accordo,
...
termina, oppure ricomincia la negoziazione inviando nuovamente la lista.

\figuralarga{../BPMN/img/alex.png}{Diagramma BPMN del processo di Alex.}{fig:bpmn-alex}
...
\textit{receive task} ``Ricevi lista disponibilit\`a''. Con uno split XOR
 Bob sceglie tra proporre data e attrezzatura, delegare la scelta
...
\figuralarga{../BPMN/img/bob.png}{Diagramma BPMN del processo di Bob.}{fig:bpmn-bob}
...
scenario \`e che i tre possibili messaggi iniziali di Bob che sono proposta, delega,
...
\figuralarga{../BPMN/img/collaboration.png}{Diagramma BPMN della collaboration tra Alex e Bob.}{fig:bpmn-collab}
...
\begin{itemize}
	...
	tipo corretto, corrispondenti alla
	...
	\item ogni message flow \`e stato tradotto in un \emph{posto di interfaccia} che collega la transizione mittente a quella ricevente.
\end{itemize}
...
se procedere su manutenzione riuscita o riaccordarsi in caso di manutenzione fallita. Poich\'e la
...
\subsection{Alex}
La rete di workflow di Alex \`e mostrata in Fig.~\ref{fig:pn-alex-1}--\ref{fig:pn-alex-2}
(divisa in due parti per leggibilit\`a). Ha 28 posti,
...
distinte, dunque ammette un S-invariante positivo; essendo sound, il coverability graph $\net^{*}$ \`e bounded e live e ammette un T-invariante positivo. Poich\'e
la rete \`e safe, il reachability graph \`e finito e coincide con il coverability graph:
 esso ha 28 vertici e 33 archi. La rete \`e \textbf{sound}.

\figuralarga{../PetriNet/img/alex_1.png}{Rete di workflow del processo di Alex (parte 1 di 2).}{fig:pn-alex-1}
\figuralarga{../PetriNet/img/alex_2.png}{Rete di workflow del processo di Alex (parte 2 di 2).}{fig:pn-alex-2}

\subsection{Bob}
La rete di workflow di Bob \`e mostrata in Fig.~\ref{fig:pn-bob-1}--\ref{fig:pn-bob-2}
(divisa in due parti per leggibilit\`a). Ha 30 posti,
...
S-component) e \textbf{sound}. Il reachability graph coincide con il coverability graph e ha 30 vertici e 35 archi.

\figuralarga{../PetriNet/img/bob_1.png}{Rete di workflow del processo di Bob (parte 1 di 2).}{fig:pn-bob-1}
\figuralarga{../PetriNet/img/bob_2.png}{Rete di workflow del processo di Bob (parte 2 di 2).}{fig:pn-bob-2}

\subsection{Collaboration (workflow system)}
Il workflow system \`e mostrato in Fig.~\ref{fig:pn-collab-1}--\ref{fig:pn-collab-2}
(diviso in due parti per leggibilit\`a). Ha 72 posti,
...
\begin{itemize}
	...
	fa da guardia su una transizione che ha anche altri posti nel
	pre-set. Sono cio\`e i punti dove avvengono i 4 message flow scambiati tra Alex e Bob: \`e
	proprio la sincronizzazione tramite message flow a rendere la rete non free-choice, e
	...
	sincronizzati tramite message flow: ogni transizione mittente genera due token (di
	controllo e di messaggio) che seguono cammini disgiunti e riconvergono a valle,
	producendo un gran numero di handle.
\end{itemize}

\figuralarga{../PetriNet/img/collaboration_1.png}{Workflow system: collaboration tra Alex e Bob con i message flow (parte 1 di 2).}{fig:pn-collab-1}
\figuralarga{../PetriNet/img/collaboration_2.png}{Workflow system: collaboration tra Alex e Bob con i message flow (parte 2 di 2).}{fig:pn-collab-2}
...
errore. La soundness del sistema \`e stata verificata direttamente sul reachability graph
che ha 199 vertici e 340 archi. Poich\'e la rete \`e bounded, il
reachability graph \`e finito e coincide con il coverability graph; essendo
...
\begin{table}[H]
	\centering
	\caption{Metriche strutturali e dimensione del reachability graph (RG).}
	\label{tab:metriche}
...
il requisito richiesto per tutti i pool e per l'intero diagramma di
collaborazione.
